// Webb Realty GA4 measurement and high-value contact events.
(()=>{const id='G-S470VH6KC5';if(!document.querySelector(`script[src*="googletagmanager.com/gtag/js?id=${id}"]`)){const s=document.createElement('script');s.async=true;s.src=`https://www.googletagmanager.com/gtag/js?id=${id}`;document.head.append(s)}window.dataLayer=window.dataLayer||[];window.gtag=window.gtag||function(){dataLayer.push(arguments)};gtag('js',new Date);gtag('config',id);document.addEventListener('click',e=>{const a=e.target.closest('a');if(!a)return;const href=a.getAttribute('href')||'';const event=href.startsWith('tel:')?'phone_click':href.startsWith('mailto:')?'email_click':/contact/i.test(href)?'contact_click':null;if(event)gtag('event',event,{link_url:href,page_location:location.href})})})();
...
const listingHref=x=>location.protocol==='file:'?`listing.html?slug=${x.slug}`:`/listings/${x.slug}/`;
const grid=document.querySelector('[data-listings]');if(grid){grid.innerHTML=listings.map(x=>`<article class="listing-tile property-card"><a href="${listingHref(x)}"><div class="property-image"><img src="assets/images/listings/${x.image}" alt="${x.title}" loading="lazy"><span class="status${/closed|pocket/i.test(x.status)?' muted':''}">${x.status}</span></div><div class="property-info"><div><p>${x.city}</p><h3>${x.title}</h3></div><strong>${x.price}</strong><p class="facts">${x.facts}</p></div></a></article>`).join('')}
const detail=document.querySelector('[data-listing-detail]');if(detail){
  const pathMatch=location.pathname.match(/^\/listings\/([^/]+)\/?$/);
  const slug=pathMatch?.[1]||new URLSearchParams(location.search).get('slug');
  const x=listings.find(y=>y.slug===slug),gallery=window.WEBB_GALLERIES?.[slug]||[];
  if(x){
    const cleanPath=`/listings/${x.slug}/`,canonical=`https://webbrealtyonline.com${cleanPath}`;
    if(location.protocol!=='file:'&&location.pathname!==cleanPath)history.replaceState(null,'',cleanPath);
    const description=`${x.title}, ${x.city}. ${x.facts}. Listed by Webb Realty.`;
    document.title=`${x.title} | Webb Realty`;
    document.querySelector('link[rel="canonical"]')?.setAttribute('href',canonical);
    document.querySelector('meta[name="description"]')?.setAttribute('content',description);
    document.querySelector('meta[property="og:title"]')?.setAttribute('content',`${x.title} | Webb Realty`);
    document.querySelector('meta[property="og:description"]')?.setAttribute('content',description);
    document.querySelector('meta[property="og:url"]')?.setAttribute('content',canonical);
    document.querySelector('meta[property="og:image"]')?.setAttribute('content',`https://webbrealtyonline.com/assets/images/listings/${x.image}`);
    document.querySelector('meta[name="twitter:title"]')?.setAttribute('content',`${x.title} | Webb Realty`);
    document.querySelector('meta[name="twitter:description"]')?.setAttribute('content',description);
    document.querySelector('meta[name="twitter:image"]')?.setAttribute('content',`https://webbrealtyonline.com/assets/images/listings/${x.image}`);
  }
  const galleryPath=image=>`assets/images/galleries/${slug}/${encodeURIComponent(image)}`;
  const contactAction='<a class="button primary" href="mailto:karen.webbrealty@gmail.com">karen.webbrealty@gmail.com</a>';
  detail.innerHTML=x?`<section class="detail-hero"><img src="assets/images/listings/${x.image}" alt="${x.title}"><div class="detail-copy"><a class="back-link" href="listings.html">← All listings</a><p class="eyebrow">${x.status}</p><h1>${x.title}</h1><p>${x.city}</p><p class="price">${x.price}</p></div></section><section class="detail-body"><div><h2>${x.title}</h2>${x.description.split('\n\n').map(t=>'<p>'+t+'</p>').join('')}<p>${x.facts}</p></div><aside class="fact-box"><strong>Interested in this property?</strong><a href="tel:+17073342633">Call or text 707-334-2633</a><a href="mailto:karen.webbrealty@gmail.com">Email Karen Webb</a>${contactAction}</aside></section>${gallery.length?`<section class="listing-gallery" aria-labelledby="gallery-heading"><div class="gallery-heading"><h2 id="gallery-heading">View Photos</h2></div><div class="gallery-grid">${gallery.map((image,index)=>`<button class="gallery-item" type="button" onclick="window.webbGalleryOpen(${index})" aria-label="View photo ${index+1} of ${gallery.length}"><img src="${galleryPath(image)}" alt="${x.title}, photo ${index+1}" loading="lazy"></button>`).join('')}</div></section><div class="gallery-dialog" hidden role="dialog" aria-modal="true" aria-label="Photo viewer"><button class="gallery-close" type="button" onclick="window.webbGalleryClose()" aria-label="Close photo viewer">×</button><button class="gallery-nav gallery-previous" type="button" onclick="window.webbGalleryMove(-1)" aria-label="Previous photo">←</button><img src="" alt=""><button class="gallery-nav gallery-next" type="button" onclick="window.webbGalleryMove(1)" aria-label="Next photo">→</button><p class="gallery-position" aria-live="polite"></p></div>`:''}`:`<div class="page-content"><div class="empty"><h1>Listing not found</h1><a href="listings.html">Return to all listings</a></div></div>`;
}
